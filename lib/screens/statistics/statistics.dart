  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        //Header title
        Center(child: Text("Summary", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 28),)),
        SizedBox(height: 40,),
        ...
        hasData
          ...
          : Expanded(child: summaryChart(screenWidth, data!),)
...
Widget summaryChart(double screenWidth, List<double> data, {bool empty = true}){
  print('EXPENSES DATA : $data');

  return empty 
  ? Center( child: Text('There are no transactions.') )
  : Center(
    child: ClayContainer(color: kLightNeutral,
      height: 300,
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Padding( padding: const EdgeInsets.only(
            ...
            child: Text( "Total Outgoing",
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600, color: Colors.black),
            ),
          ),
          Padding( padding: const EdgeInsets.symmetric( horizontal: 16.0, ),
            child: Text( '\$${data.fold(0, (i, j) => i + int.parse(j.toString())).toStringAsFixed(2)}',
...
//Overiew of expenses by month
Widget monthlyTransactions(List<String> months, List<TransactionRecord> transactions){
      ...
      itemBuilder: (BuildContext context, int index){
        return Column(
          children: <Widget>[
              ...
              child: Row(
                ...
                children: <Widget>[
                  ...
                  Text("SAVE \$${
                      (
                        transactions
                        .where((t) => DateFormat.MMMM().format(t.date) == months[index])
                        .where((t) => t.type == "income")
                        .toList()
                        .fold(0.0, (i, j) => i + j.amount)
                        -
                        (transactions
                        .where((t) => DateFormat.MMMM().format(t.date) == months[index])
                        .where((t) => t.type == "expense")
            ...
            ListView.builder(
              itemCount: (transactions.where((t) => DateFormat.MMMM().format(t.date) == months[index]).toList()).length,
              physics: ClampingScrollPhysics(), 
              shrinkWrap: true,
              itemBuilder: (BuildContext ctxt, int i){
                return TransactionItem(transaction: (sortedList!.where((t) => DateFormat.MMMM().format(t.date) == months[index]).toList())[i],);
              },
            )
